	...
	class Persona
	{
		...
		public:
			...
			*/			
			...
			//Ahora los de carga y descarga de fichero

			/*!
				\fn 	friend std::ofstream & operator<<(std::ofstream &out, const Persona &p);
				\brief 	Escribe los datos de una persona a un fichero
				\param 	out Fichero de salida
				\param 	p Persona a escribir en el fichero
				\return Flujo de salida en el que se han escrito los datos de la Persona.
				\sa 	friend std::ifstream & opreator>>(std::ifstream &in, const Persona &p);
			*/

			friend std::ofstream & operator<<(std::ofstream &stream, const Persona &p);

			/*!
				\fn 	friend std::ifstream & opreator>>(std::ifstream &in, const Persona &p);
				\brief 	Lee desde un flujo de fichero una persona
				\param 	in Fichero de entrada
				\param 	p Persona a escribir en el fichero
				\return Flujo de salida en el que se han escrito los datos de la Persona
				\sa 	friend std::ofstream & operator<<(std::ofstream &out, const Persona &p);
			*/
