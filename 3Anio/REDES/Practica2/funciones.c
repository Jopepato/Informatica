...
int ** generaCarton(){
	...
	for(i=0; i<3;i++){
		...
		while(k!=4){
			quita = rand()%9;
			if(!compruebaElementoVector(quitados, quita)){
				quitados[k] = quita;
				k++;
				carton[i][quita] = 0;
			}
...
void guardaUsuarioPassword(char ** usuario, char ** password){
	...
	FILE *fichero;
	if(file = fopen("Datos.txt", "a")==NULL){
		printf("Fichero especificado invalido\n");
	}

	//Ahora escribimos el usuario y la pass en el fichero
	fprintf(fichero, "%s %s\n",usuario, password);
